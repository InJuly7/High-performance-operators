...
__global__ void flash_attn_v1(float *Q, float *K, float *V, float *l, float *m, float *O, int S, int dk, int Tc, int Tr, int Bc, int Br) {
...
int main() {
    ...
    for(int iter = 0; iter < 5; iter++) {
        Perf("flash_attn_v1");
        flash_attn_v1<sram_size><<<grid, block>>>(d_Q, d_K, d_V, d_l, d_m, d_O, S1, dk, Tc, Tr, Bc, Br);
    }
