...
__global__ void sgemm_v3_t_8x8_AT_f32(float *mat_A, float *mat_B, float *mat_C, int M, int K, int N) {
    ...
    for (int k = 0; k < K; k += BK) {
        ...
        cudaLog("Store SMemB[%d,%d]\n", ST_SMemB_row, ST_SMemB_col);
...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
        sgemm_v3_t_8x8_AT_f32<BM, BK, BN, TM, TN><<<grid, block>>>(mat_A_device, mat_B_device, mat_C_device, M, K, N);
