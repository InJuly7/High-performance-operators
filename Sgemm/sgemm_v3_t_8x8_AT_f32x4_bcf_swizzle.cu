...
__global__ void sgemm_v3_t_8x8_AT_f32x4_bcf_swizzle(float *mat_A, float *mat_B, float *mat_C, int M, int K, int N) {
    ...
    for (int k = 0; k < K; k += BK) {
        ...
        for (int bk = 0; bk < BK; bk++) {
            ...
            for (int tn = 0; tn < TN; tn++) LD_SMem_Reg_B[tn] = SMem_B[bk][(tn * 16) + LDST_swizzle_col_B];
