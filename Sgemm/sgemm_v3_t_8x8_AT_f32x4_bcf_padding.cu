...
__global__ void sgemm_v3_t_8x8_AT_f32x4_bcf_padding(float *mat_A, float *mat_B, float *mat_C, int M, int K, int N) {
    ...
    __shared__ float SMem_B[BK][BN + padding];
    // (BM * BK) / (blockDim.x) == 4;
    ...
    for (int k = 0; k < K; k += BK) {
        ...
        FLOAT4(SMem_B[LDST_row_B][LDST_col_B]) = FLOAT4(ST_SMem_Reg_B[0]);
...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
        sgemm_v3_t_8x8_AT_f32x4_bcf_padding<BM, BK, BN, TM, TN><<<grid, block>>>(mat_A_device, mat_B_device, mat_C_device, M, K, N);
