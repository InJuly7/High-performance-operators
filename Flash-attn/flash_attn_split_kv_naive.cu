...
__global__ void flash_attn_split_kv_naive(float *Q, float *K, float *V, float *l, float *m, float *O, int S, int dk, int Tc, int Tr, int Bc, int Br) {
...
int main() {
    ...
    dim3 grid(H);
    dim3 block(Bc);
    flash_attn_split_kv_naive<sram_size><<<grid, block>>>(d_Q, d_K, d_V, d_l, d_m, d_O, S1, dk, Tc, Tr, Bc, Br);
