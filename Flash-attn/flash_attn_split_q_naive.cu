...
__global__ void flash_attn_split_q_naive(float *Q, float *K, float *V, float *l, float *O, int S, int dk, int Tc, int Tr, int Bc, int Br) {
...
int main() {
    ...
    dim3 grid(H, Tr);
    dim3 block(Br);
    flash_attn_split_q_naive<sram_size><<<grid, block>>>(d_Q, d_K, d_V, d_l, d_O, S1, dk, Tc, Tr, Bc, Br);
