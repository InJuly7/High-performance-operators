...
__global__ void cuda_sgemm(float *matrix_A_device, float *matrix_B_device, float *matrix_C_device) {
    ...
    for (int i = 0; i < K; i += BK) {
        ...
        for(int j = 0; j < LOAD_A_SMEM_NUM; j+=4) {
            ...
            A_smem[A_smem_x + j][A_smem_y] = load_reg[0];
            A_smem[A_smem_x + j + 1][A_smem_y] = load_reg[1];
            A_smem[A_smem_x + j + 2][A_smem_y] = load_reg[2];
            A_smem[A_smem_x + j + 3][A_smem_y] = load_reg[3];
        }
        ...
        for (int j = 0; j < BK; j++) {
            FETCH_FLOAT4(row_frag[0]) = FETCH_FLOAT4(A_smem[j][c_row_start]);
    ...
    for (int i = 0; i < ROW_FRAG; i++) {
        FETCH_FLOAT4(C_ptr_start[(c_row_start + i) * N + c_col_start]) = FETCH_FLOAT4(res[i][0]);
    }
}
