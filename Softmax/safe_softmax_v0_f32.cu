...
template <unsigned int NUM_THREADS>
__global__ void safe_softmax_v0_f32(float *mat_A, float *mat_B, int N) {
...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
        Perf perf("safe_softmax_v0_f32");
        safe_softmax_v0_f32<N2><<<grid, block>>>(mat_A_device, mat_B_device, N2);
    }
