...
class Config:
    def __init__(self):
        ...
        self.kernels = {
            "softmax_f32": 0.0,
            "softmax_f32x4": 0.0,
            "safe_softmax_f32": 0.0,
            "safe_softmax_f32x4": 0.0,
            "online_softmax_f32": 0.0,
            "online_softmax_f32x4": 0.0,
            "safe_softmax_f16": 0.0,
            "safe_softmax_f16x8": 0.0,
            "safe_softmax_f16_pack": 0.0,
            "online_softmax_f16x8": 0.0,
            "torch_safe_softmax": 0.0,
        ...
        self.lib = load(
            ...
            sources=[
                "./Softmax/bind.cpp",
                "./Softmax/softmax_f32.cu",
                "./Softmax/softmax_f32x4.cu",
                "./Softmax/safe_softmax_f32.cu",
                "./Softmax/safe_softmax_f32x4.cu",
                "./Softmax/online_softmax_f32.cu",
                "./Softmax/online_softmax_f32x4.cu",
                "./Softmax/safe_softmax_f16.cu",
                "./Softmax/safe_softmax_f16x8.cu",
                "./Softmax/safe_softmax_f16_pack.cu",
                "./Softmax/online_softmax_f16x8.cu",
            ],  # 这里可以根据需要添加更多的源文件
            extra_cuda_cflags=[
                "-O3",
...
"""
@func: 使用torch框架实现safe-softmax, 并测试性能
@param: A : 输入Tensor
@param: B : 输出Tensor
@param: shape : 形状
@param: config: 配置 
...
if __name__ == "__main__":
    ...
    for dim1 in S:
        for dim2 in H:
            ...
            safe_softmax_torch(A, B, shape=[dim1, dim2], config=config)
            ...
            if dim2 <= 1024:
                run_benchmark(lib.softmax_f32, A, B, "softmax_f32", config=config)
                run_benchmark(lib.safe_softmax_f32, A, B, "safe_softmax_f32", config=config)
                run_benchmark(lib.online_softmax_f32, A, B, "online_softmax_f32", config=config)

            if dim2 >= 128 and dim2 <= 4096:
                run_benchmark(lib.softmax_f32x4, A, B, "softmax_f32x4", config=config)
                run_benchmark(lib.safe_softmax_f32x4, A, B, "safe_softmax_f32x4", config=config)
                run_benchmark(lib.online_softmax_f32x4, A, B, "online_softmax_f32x4", config=config)
            print_perf(
    ...
    for dim1 in S:
        for dim2 in H:
            ...
            if dim2 >= 32 and dim2 <= 1024:
                run_benchmark(lib.safe_softmax_f16, A, B, "safe_softmax_f16", config=config)
            if dim2 >= 256 and dim2 <= 8192:
                run_benchmark(lib.safe_softmax_f16x8, A, B, "safe_softmax_f16x8", config=config)
                run_benchmark(lib.safe_softmax_f16_pack, A, B, "safe_softmax_f16_pack", config=config)
                run_benchmark(lib.online_softmax_f16x8, A, B, "online_softmax_f16x8", config=config)
            print_perf(
                input_shape=[dim1, dim2],
                output_shape=[dim1, dim2],
                config=config,
