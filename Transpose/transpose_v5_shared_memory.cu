...
__global__ void mat_transpose_kernel_v5(float *input, float *output, int M, int N) {
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 5; i++) {
        Perf perf("mat_transpose_kernel_v5");
        constexpr int BLOCK_SZ = 16;
        dim3 block(BLOCK_SZ, BLOCK_SZ);
        dim3 grid((MATRIX_N + BLOCK_SZ - 1) / BLOCK_SZ, (MATRIX_M + BLOCK_SZ - 1) / BLOCK_SZ);
        mat_transpose_kernel_v5<BLOCK_SZ><<<grid, block>>>(input_device, output_device, MATRIX_M, MATRIX_N);
