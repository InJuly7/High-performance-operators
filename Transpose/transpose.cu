...
__global__ void transpose_col4row1(const float *input, float *__restrict__ output) {
...
__global__ void transpose_col2row1(const float *input, float *__restrict__ output) {
    int col_idx = blockIdx.x * blockDim.x + threadIdx.x;
    int row_idx = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void transpose_float2(const float *input, float *__restrict__ output) {
    int col_idx = blockIdx.x * blockDim.x + threadIdx.x;
    int row_idx = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void transpose_perfect(const float *input, float *__restrict__ output) {
    int col_idx = blockIdx.x * blockDim.x + threadIdx.x;
    int row_idx = blockIdx.y * blockDim.y + threadIdx.y;
...
int main(int argc, char *argv[]) {
...

    {
        ...
        cudaMemcpy(matrix_output, d_matrix_output, MATRIX_M * MATRIX_N * sizeof(float), cudaMemcpyDeviceToHost);
    }
...

    {
        ...
        cudaDeviceSynchronize();
        cudaMemcpy(cpu_perfect, perfect_output, MATRIX_M * MATRIX_N * sizeof(float), cudaMemcpyDeviceToHost);
    }
    ...
    float *cpu_floatv2 = new float[MATRIX_M * MATRIX_N];
    {
        ...
        cudaDeviceSynchronize();
        cudaMemcpy(cpu_floatv2, floatv2_output, MATRIX_M * MATRIX_N * sizeof(float), cudaMemcpyDeviceToHost);
    }
    ...
    float *cpu_float_col4row1 = new float[MATRIX_M * MATRIX_N];
    {
        ...
        cudaDeviceSynchronize();
        cudaMemcpy(cpu_float_col4row1, float_col4row1, MATRIX_M * MATRIX_N * sizeof(float), cudaMemcpyDeviceToHost);
    }
    std::cout << std::endl;
    ...
    float *cpu_float_col2row1 = new float[MATRIX_M * MATRIX_N];
    {
        ...
        cudaDeviceSynchronize();
        cudaMemcpy(cpu_float_col2row1, float_col2row1, MATRIX_M * MATRIX_N * sizeof(float), cudaMemcpyDeviceToHost);
    }
    std::cout << std::endl;
    ...
    // cpu_csdn_shm = perfect_output;
    {
        ...
        cudaMemset(csdn_shm, 0, MATRIX_M * MATRIX_N * sizeof(float));
        {
            ...
            mat_transpose_kernel_v3<BLOCK_SZ, NUM_PER_THREAD><<<grid_size, block_size>>>(d_matrix, csdn_shm, MATRIX_M, MATRIX_N);
            cudaDeviceSynchronize();
            cudaMemcpy(cpu_csdn_shm, csdn_shm, MATRIX_M * MATRIX_N * sizeof(float), cudaMemcpyDeviceToHost);
        }
    }
