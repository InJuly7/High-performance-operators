...
// y=y'*g (g: scale)
template <unsigned int NUM_THREADS>
__global__ void rms_norm_f16_kernel(half* A, half* B, half* C, int K) {
    half* A_start = A + blockIdx.x * K;
    half* C_start = C + blockIdx.x * K;
    const float epsilon = 1e-5f;

    // 块内共享, 求出当前行 rsqrtf(sum(ai^2)/K)
    __shared__ float s_variance;
    float g = __half2float(B[threadIdx.x]);
    float value = __half2float(A_start[threadIdx.x]);
    float variance = value * value;
    variance = block_reduce_sum_f32<NUM_THREADS>(variance);
    if (threadIdx.x == 0) s_variance = rsqrtf(variance / K + epsilon);
    __syncthreads();
    C_start[threadIdx.x] = __float2half(value * s_variance * g);
}

#define LAUNCH_RMS_NORM_F16(H) \
    rms_norm_f16_kernel<(H)>   \
        <<<grid, block>>>(reinterpret_cast<half*>(A.data_ptr()), reinterpret_cast<half*>(B.data_ptr()), reinterpret_cast<half*>(C.data_ptr()), (H));

#define DISPATCH_RMS_NORM_F16(S, H)                                     \
    dim3 block((H));                                                    \
    dim3 grid((S));                                                     \
    switch ((H)) {                                                      \
        case 32:                                                        \
            LAUNCH_RMS_NORM_F16(32) break;                              \
        case 64:                                                        \
            LAUNCH_RMS_NORM_F16(64) break;                              \
        case 128:                                                       \
            LAUNCH_RMS_NORM_F16(128) break;                             \
        case 256:                                                       \
            LAUNCH_RMS_NORM_F16(256) break;                             \
        case 512:                                                       \
            LAUNCH_RMS_NORM_F16(512) break;                             \
        case 1024:                                                      \
            LAUNCH_RMS_NORM_F16(1024) break;                            \
        default:                                                        \
