...
// y=y'*g (g: scale)
template <unsigned int NUM_THREADS>
__global__ void rms_norm_v0_f32(float *mat_A, float *mat_B, float g, int N, int K) {
...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
        Perf perf("rms_norm_v0_f32");
        rms_norm_v0_f32<K><<<grid, block>>>(mat_A_device, mat_B_device, g, N, K);
