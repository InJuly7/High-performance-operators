...
// y=y'*g (g: scale)
template <unsigned int NUM_THREADS>
__global__ void rms_norm_f32_kernel(float* mat_A, float* mat_B, float g, int K) {
...
#define LAUNCH_RMS_NORM_F32(H) \
    rms_norm_f32_kernel<(H)><<<grid, block>>>(reinterpret_cast<float*>(A.data_ptr()), reinterpret_cast<float*>(B.data_ptr()), g, (H));
...
void rms_norm_f32(torch::Tensor A, torch::Tensor B, float g) {
