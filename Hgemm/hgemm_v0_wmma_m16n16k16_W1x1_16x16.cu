...
__global__ void hgemm_v0_wmma_m16n16k16_W1x1_16x16(half *A, half *B, half *C, const int M, const int K, const int N) {
    A += blockIdx.y * WM * K;
    B += blockIdx.x * WN;
...
int main() {
    ...
    dim3 block(32);
    for(int i = 0; i < 5; i++) {
        Perf("hgemm_v0_wmma_m16n16k16_W1x1_16x16");
        hgemm_v0_wmma_m16n16k16_W1x1_16x16<WM, WK, WN><<<grid, block>>>(mat_A_device, mat_B_device, mat_C_wmma, M, K, N);
    }
