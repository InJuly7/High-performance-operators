...
__global__ void hgemm_v1_wmma_m16n16k16_4x2(half *A, half *B, half *C, const int M, const int K, const int N) {
    A += blockIdx.y * WM * K;
    B += blockIdx.x * WN;
    C += blockIdx.y * WM * N + blockIdx.x * WN;
    const int Tk = CEIL_DIV(K, WK);
    ...
    for (int tk = 0; tk < Tk; tk++) {
        wmma::load_matrix_sync(A_frag, A + tk * WK, K);
        wmma::load_matrix_sync(B_frag, B + tk * WK * N, N);
...
int main() {
    ...
    dim3 block(32);
    hgemm_v1_wmma_m16n16k16_4x2<WM, WK, WN, TM, TN><<<grid, block>>>(mat_A_device, mat_B_device, mat_C_wmma, M, K, N);
