...
__global__ void hgemm_v1_wmma_m16n16k16_W4x2_T1x1(half *A, half *B, half *C, const int M, const int K, const int N) {
    ...
    int LD_SMemA_Row = (warpId * 16) / 32;
    int LD_SMemB_Col = (warpId * 16) & 31;
    ...
    wmma::store_matrix_sync(&C[LD_SMemA_Row * WM * N + LD_SMemB_Col], C_frag, N, wmma::mem_row_major);
}
...
int main() {
    ...
    dim3 block(256);
    for (int i = 0; i < 1; i++) {
        Perf("hgemm_v1_wmma_m16n16k16_W4x2_T1x1");
        hgemm_v1_wmma_m16n16k16_W4x2_T1x1<WM, WK, WN, TM, TN><<<grid, block>>>(mat_A_device, mat_B_device, mat_C_wmma, M, K, N);
    }
