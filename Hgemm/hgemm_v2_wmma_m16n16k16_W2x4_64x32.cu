...
__global__ void hgemm_v2_wmma_m16n16k16_W1x8_128x16(half *A, half *B, half *C, const int M, const int K, const int N) {
    ...
    for (int k = 0; k < K; k += BK) {
        ...
        for (int tm = 0; tm < TM; tm++) {
            ...
            wmma::mma_sync(C_frag[tm][warpId], A_frag, B_frag, C_frag[tm][warpId]);
            __syncthreads();
        }
    }
    for (int tm = 0; tm < TM; ++tm) {
        wmma::store_matrix_sync(&C[tm * WM * N + warpId * WN], C_frag[tm][warpId], N, wmma::mem_row_major);
    }
}
...
int main() {
    ...
    dim3 block(256);
    hgemm_v2_wmma_m16n16k16_W1x8_128x16<WM, WK, WN, TM, TN><<<grid, block>>>(mat_A_device, mat_B_device, mat_C_wmma, M, K, N);
