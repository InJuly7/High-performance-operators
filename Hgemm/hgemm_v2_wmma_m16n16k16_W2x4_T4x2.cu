...
__global__ void hgemm_v2_wmma_m16n16k16_W2x4_T4x2(half *A, half *B, half *C, const int M, const int K, const int N) {
    ...
    for (int k = 0; k < K; k += BK) {
        ...
        for (int tm = 0; tm < TM; tm++) {
            wmma::load_matrix_sync(A_frag[tm], &SMem_A[LD_SMemA_Row + tm * WM][0], BK);
        }
        for (int tn = 0; tn < TN; tn++) {
            wmma::load_matrix_sync(B_frag[tn], &SMem_B[0][LD_SMemB_Col + tn * WN], BN);
        }
        for (int tm = 0; tm < TM; tm++) {
            for (int tn = 0; tn < TN; tn++) {
